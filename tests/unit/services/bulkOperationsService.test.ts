...
describe('BulkOperationsService', () => {
  ...
  describe('batchApproveSuggestions', () => {
    it('should approve pending AI suggestions', async () => {
      const { getDocs } = require('firebase/firestore');
          ...
          },
          {
            ...
            data: () => ({
              ...
              metadata: {
                ...
                suggestionUsed: true, // Already used, should not be included
              },
            }),
          },
        ],
      ...
      const result = await bulkOperationsService.batchApproveSuggestions('user123');

      expect(result.totalProcessed).toBe(2); // msg1 and msg2 are pending
      expect(result.successCount).toBe(2);
      expect(result.failureCount).toBe(0);
      expect(result.completed).toBe(true);
    });
    ...
    it('should limit to 100 items', async () => {
      ...
      // Create 150 pending suggestions
      const mockMessages = {
        docs: Array.from({ length: 150 }, (_, i) => ({
          ...
          data: () => ({
            senderId: 'user123',
            metadata: {
              suggestedResponse: `Suggestion ${i}`,
            },
          }),
        })),
      };
      ...
      expect(result.totalProcessed).toBe(100); // Limited to 100
      expect(result.errors.length).toBeGreaterThan(0); // Should have warning about limit
    });

    it("should only process user's own suggestions", async () => {
      const { getDocs } = require('firebase/firestore');
      ...
      // Firestore query filters by senderId, so only user123's messages are returned
      const mockMessages = {
        docs: [
          {
            id: 'msg1',
            data: () => ({
              senderId: 'user123',
              metadata: { suggestedResponse: 'Suggestion' },
            }),
          },
        ],
      };
      ...
      // Should only process msg1 (Firestore query filters by senderId)
      expect(result.totalProcessed).toBe(1);
    });
    ...
    it('should call progress callback', async () => {
      ...
      const progressCallback = jest.fn();
      await bulkOperationsService.batchApproveSuggestions('user123', progressCallback);
