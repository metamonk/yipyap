 ...
 * distracting from message content. Lines adapt to theme with low opacity
 * for a subtle appearance in both light and dark modes.
 *
 * @example
 * ```tsx
...
export const DateSeparator: FC<DateSeparatorProps> = memo(({ timestamp }) => {
  ...
  return (
    <View style={styles.container} testID="date-separator">
      ...
      <Text style={dynamicStyles.dateText}>{formatDateSeparator(timestamp)}</Text>
      <View style={dynamicStyles.line} />
    </View>
  );
});
